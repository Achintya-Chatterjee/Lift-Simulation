...
function resetSimulation() {
  ...
  const floorsCountContainer = document.querySelector("#floors_count");
  const liftsCountContainer = document.querySelector("#lifts_count");
  floorsCountContainer.textContent = "";
  liftsCountContainer.textContent = "";
...
function handleLiftCall(event) {
    ...
    // Search for the closest available lift
    for (let [liftId, isAvailable] of liftsAvailability) {
      if (isAvailable) {
        const liftFloor = liftAt.get(liftId);
        const distance = Math.abs(liftFloor - floorNumber);
